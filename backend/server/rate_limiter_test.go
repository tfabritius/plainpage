...
func TestRateLimiter_Allow(t *testing.T) {
	burst := 3
	limiter := NewRateLimiter(burst, rate.Limit(10), 1*time.Minute)
	key := "test_key"
...
func TestRateLimiter_TTL(t *testing.T) {
	...
	limiter := NewRateLimiter(1, rate.Limit(10), ttl)
	key := "test_key"
...
func TestRateLimiter_Concurrent(t *testing.T) {
	...
	limiter := NewRateLimiter(burst, rate.Limit(0), 1*time.Minute)
	key := "test_key"
	numRequests := 100
	...
	assert.Equal(t, burst, allowedCount, "expected %d allowed requests", burst)

	// After more requests than the burst size, the limiter should be exhausted.
